...
class Reader(object):
    ...
    def initialize(self):
        ...
        if self._device is None:
            sys.exit('RFID Reader not found. Check idVendor and idProduct')
            ...
            try:
                ...
                if e.args[0] == 110 and data_read:
                    if len(data) < self.data_size:
                        sys.exit(
                            'Got %s bytes instead of %s - %s' % (len(data), self.data_size, str(data)))
                    else:
                        break
    ...
    def get_chunked_data(self, raw_data):
        return Map.chunk_data(raw_data, self.chunk_size)

    @staticmethod
    def raw_data_to_keys(extracted_data):
        return ''.join(map(Map.raw_to_key, extracted_data))
